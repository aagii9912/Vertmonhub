...
Marketing performance extension (2026-09-21): `/marketing` now combines project/channel/activity Lead–Sales–Deal reporting, monthly marketing-manager/project targets, activity and spend entry, lead attribution, Excel export and AI `get_marketing_performance`. Shared calculation/loader: `lib/marketing/performance.ts` and `performance-load.ts`. Requires additive migration `20260921120000_marketing_performance.sql` before deployment; not applied to production by this implementation task. `sales_handoff_at` records first assignment across write paths; existing dates are never inferred. Exact Meta campaign-ID mappings stamp future lead attribution. See `docs/MARKETING-PERFORMANCE-2026-09-21.md` for cohort definitions, role checks, rollout and test commands.

Meeting-driven marketing analytics layer (migration `20260721140000`):
- **Budget** (`/marketing/budget`): `marketing_budgets` (monthly plan per shop) + `marketing_spend_entries` (manual spend log by channel — billboard/radio/boosts; canonical channel list `SPEND_CHANNELS` in `lib/marketing/budget.ts`). `GET/PUT/POST/DELETE /api/marketing/budget` — GET compares budget vs spend vs **contract revenue** (`manager_monthly_sales`, same source as dashboards) with color rule <80% ok/green, 80–100% warn/yellow, >100% over/red (`budgetStatus`, unit-tested). Meta daily spend now joins the canonical spend loader after `20260921140000_meta_daily_spend.sql`: account-local dates, explicit FX, atomic paginated imports and manual Meta overlap exclusion on covered days. The 30-day `ad_campaigns.spend` snapshot is not added. Cron runs every 6 hours; `/marketing` provides manual backfill/status/FX controls. See `docs/META-DAILY-SPEND-2026-09-21.md` for the overlap policy, rollout and validation; production rollout remains pending.
