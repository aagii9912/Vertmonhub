...
### Үе 1 — Мөнгө ба өгөгдлийн бүрэн бүтэн байдал ✅ ХИЙГДСЭН
...
### Үе 3 — Route шилжүүлэлт ✅ ХИЙГДСЭН
...
### Үе 4 — Харагдац ба хамрах хүрээг гүйцээх ✅ ХИЙГДСЭН
...
### Үе 5 — Нэгтгэл ба тогтвортой байдал ✅ ХИЙГДСЭН
...
| | Аудитын үед | Одоо |
|---|---|---|
| Нэвтрэлт/эрхийн шалгалттай route | 108 / 132 | **134 / 134** (CI-аар хамгаалагдсан) |
| Модулийн эрхийн шалгалттай | 36 | **59** + `apiContext` |
| Аудит бичдэг mutating route | 5 / 86 | **49 / 86** (+37 шалтгаантай алгасалт) |
| Аудит суваг | 5 салангид | **1 нэгдсэн** (+`audit_unified` view) |
| before/after хадгалалт | ❌ | ✅ |
| IP / user-agent | ❌ | ✅ |
| Өөрчлөгдөшгүй лог | ❌ | ✅ (trigger) |
| Хадгалах хугацаа | ❌ | ✅ 24 сар |
| Удирдлагын харагдац | зөвхөн AI | **`/dashboard/audit` + энтити тус бүрийн түүх** |
| Эрхгүй оролдлого бүртгэх | ❌ | ✅ `status='denied'` |
| Нэвтрэлтийн зам | 3 зэрэгцээ | **1** |
| Нэвтрэлтийн үйл явдал бүртгэх | ❌ | ✅ |
| Экспортын бүртгэл | ❌ | ✅ + модулийн эрх |
| Алдааны хяналт (Sentry) | үхмэл | **ажиллагаатай** |
| Гэрээ устгалт | hard, ул мөргүй | **soft, бүртгэлтэй** |
| Гэрээний төлбөрийн нийлбэр | шинэчлэгддэггүй | **DB trigger-ээр атомаар** |
| /api/health | env шалгаад 200 | **DB-д бодит probe** |
| CI хаалга | lint continue-on-error | **lint + аудит хамрах хүрээ блоклоно** |
| Тест | 275 | **286** |

### Хийгдээгүй үлдсэн (шалтгаантай)

| Ажил | Шалтгаан |
|---|---|
| `user_roles`-д `shop_id` (дүр төсөл тус бүрээр) | `UNIQUE(user_id)`-г өөрчлөх нь бүх дүрийн уншилтад нөлөөлнө. CLAUDE.md-д олон-түрээслэгчийн бүрэн шинэчлэлийг тусдаа төлөвлөгөө гэж заасан |
| 37 mutating route-ын аудит | Cron-ийн уншилт/мэдэгдэл, webhook (эзлэхүүн), AI (өөрийн сувагтай), UI тохиргоо. `scripts/check-audit-coverage.mjs`-ийн ALLOWLIST-д шалтгаантайгаар бүртгэсэн |
| Түүхэн `paid_amount` тулгалт | Урьдчилгаа `payment_schedules`-д орсон эсэх нь төсөл бүрд өөр тул автомат засвар давхар тоолох эрсдэлтэй. Migration дотор шалгах query үлдээсэн — нөөцлөлт хийсний дараа гараар |
| Түүхэн нууц үг эргүүлэх | Кодоос хийх боломжгүй — Supabase дээр гараар солино |

**Дараагийн алхам:** migration-уудыг production-д тавих, `.claude/settings.local.json`-д байсан DB нууц үгийг Supabase дээр солих.
